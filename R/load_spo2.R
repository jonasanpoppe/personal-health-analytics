...
load_spo2 <- function(
  ...
  csv_files <- list.files(
    dir,
    pattern    = "^daily_oxygen_saturation_.*\\.csv$",
    full.names = TRUE
  )
  ...
  daily_data <- raw_data %>%
    ...
    summarise(
      .by = date,
      spo2_avg = mean(average_percentage, na.rm = TRUE)
    ) %>%
    arrange(date)

  DBI::dbWriteTable(con, "fitbit_spo2_daily", daily_data, overwrite = TRUE)
  message("\u2713 fitbit_spo2_daily written (", nrow(daily_data), " rows)")

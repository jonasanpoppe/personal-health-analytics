...
load_steps <- function(
  ...
  if (length(csv_files) == 0) {
    stop("No steps CSV files found in: ", dir)
  }
  ...
  DBI::dbWriteTable(con, "fitbit_steps_daily", daily_data, overwrite = TRUE)
  message("✓ fitbit_steps_daily written (", nrow(daily_data), " rows)")

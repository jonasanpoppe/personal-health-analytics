...
load_calories <- function(
  ...
  csv_files <- list.files(
    dir,
    pattern    = "^calories_.*\\.csv$",
    full.names = TRUE
  )

  if (length(csv_files) == 0) {
    stop("No calories CSV files found in: ", dir)
  }
  ...
  DBI::dbWriteTable(con, "fitbit_calories_daily", daily_data, overwrite = TRUE)
  message("✓ fitbit_calories_daily written (", nrow(daily_data), " rows)")
